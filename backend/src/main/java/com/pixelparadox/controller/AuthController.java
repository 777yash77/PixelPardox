...
public class AuthController {
    ...
    public ResponseEntity<?> registerTeam(@RequestBody RegisterRequest request) {
        ...
        if (request.teamSize() != null && (request.teamSize() < 1 || request.teamSize() > 2)) {
            return ResponseEntity.badRequest().body(Map.of("message", "Team size must be strictly 1 or 2 members."));
        }

        int size = (request.teamSize() != null && request.teamSize() >= 1 && request.teamSize() <= 2) ? request.teamSize() : 1;
        User user = new User(
                cleanTeamName,
                cleanTeamId,
