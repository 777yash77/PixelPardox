...
public class AuthController {
    ...
    public ResponseEntity<?> registerTeam(@RequestBody RegisterRequest request) {
        ...
        int extraCount = (request.memberNames() != null) ? request.memberNames().size() : 0;
        int totalMembers = 1 + 1 + extraCount;
        ...
        if (request.memberNames() != null && !request.memberNames().isEmpty()) {
            user.setMemberNames(new ArrayList<>(request.memberNames()));
        }
        userRepository.save(user);

        String debugOtp = otpService.generateAndSendOtp(user);
        Map<String, Object> resp = new HashMap<>();
        resp.put("message", "Registration successful. Please verify OTP sent to your email.");
        if (debugOtp != null) resp.put("debugOtp", debugOtp);
...
"@
Set-Content -Path "AuthController.java" -Value $content -Encoding UTF8
Write-Host "AuthController written"
