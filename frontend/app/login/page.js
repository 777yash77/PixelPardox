...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem('teamSize', data.teamSize != null ? String(data.teamSize) : '1')
        localStorage.setItem('memberNames', data.memberNames || '')

        const rawMembers = data.memberNames ? data.memberNames.split(',').map(s => s.trim()).filter(Boolean) : []
        ...
        if (rawMembers.length === 0) {
          const defaultParticipant = data.teamName || data.teamId || 'Member'
          localStorage.setItem('participantName', defaultParticipant)
          setParticipantName(defaultParticipant)
          router.push('/game')
          return
        }

        if (rawMembers.length === 1) {
          localStorage.setItem('participantName', rawMembers[0])
          setParticipantName(rawMembers[0])
          router.push('/game')
          return
        }

        // Multiple members registered: clear any previous squad participant and prompt step 2
        localStorage.removeItem('participantName')
        setParticipantName('')
        setMemberNamesList(rawMembers)
        setStep(2)
      } else {
  ...
  const handleSelectParticipant = (e) => {
    e.preventDefault()
    const targetName = participantName.trim() || (memberNamesList.length > 0 ? memberNamesList[0] : (localStorage.getItem('teamName') || 'Member'))
    localStorage.setItem('participantName', targetName)
    router.push('/game')
  }
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: '14px' }}>
                ...
                <div className="auth-briefing-item" style={{ gridColumn: '1 / -1' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
                    <span style={{ fontSize: '1.15rem' }}>🛡️</span>
                    <span style={{ fontSize: '0.86rem', fontWeight: 800, color: '#7DD3FC', letterSpacing: '0.5px' }}>ANTI-CHEAT PROTOCOL</span>
                    <span style={{ marginLeft: 'auto', background: 'rgba(56, 189, 248, 0.2)', border: '1px solid rgba(56, 189, 248, 0.45)', borderRadius: '4px', padding: '2px 8px', fontSize: '0.7rem', color: '#FFF', fontWeight: 700 }}>SECURE EXAM</span>
                  </div>
                  <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', margin: 0, lineHeight: 1.45 }}>
                    Session integrity and tab switches are monitored in real time. Maintain focus on the active exam window.
                  </p>
                </div>
              </div>
            </div>
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ background: 'rgba(56, 189, 248, 0.08)', border: '1px solid rgba(56, 189, 248, 0.3)', borderRadius: '10px', padding: '14px 16px', marginBottom: '22px' }}>
                    ...
                    <p style={{ fontSize: '0.86rem', color: '#FFF', margin: 0, lineHeight: '1.5' }}>
                      All registered team members in Team <strong>{(formData.teamId || localStorage.getItem('teamId') || '').toUpperCase()}</strong> attempt Stage 0 independently. Your individual score directly adds into your team's total leaderboard score.
                    </p>
                  </div>

                  <form onSubmit={handleSelectParticipant}>
                    <div style={{ marginBottom: '24px', textAlign: 'left' }}>
                      <span style={{ fontSize: '0.78rem', color: '#7DD3FC', fontWeight: 'bold', letterSpacing: '0.8px', display: 'block', marginBottom: '12px', textTransform: 'uppercase', textAlign: 'center' }}>
                        SELECT YOUR REGISTERED TEAMMATE PROFILE <span style={{ color: '#FF6B6B' }}>*</span>
                      </span>

                      {memberNamesList.length > 0 ? (
                        <div style={{ display: 'grid', gridTemplateColumns: memberNamesList.length === 1 ? '1fr' : 'repeat(auto-fit, minmax(130px, 1fr))', gap: '10px' }}>
                          {memberNamesList.map((name, idx) => {
                            ...
                            return (
                              <button
                                ...
                                style={{
                                  padding: '14px 12px',
                                  borderRadius: '10px',
                                  border: isChosen ? '2px solid #38BDF8' : '1px solid rgba(255,255,255,0.15)',
                                  background: isChosen ? 'rgba(56, 189, 248, 0.22)' : 'rgba(255,255,255,0.04)',
                                  color: isChosen ? '#FFF' : 'var(--text-secondary)',
                                  boxShadow: isChosen ? '0 0 15px rgba(56, 189, 248, 0.35)' : 'none',
                                  cursor: 'pointer',
                                  display: 'flex',
                                  flexDirection: 'column',
                                  alignItems: 'center',
                              ...
                              >
                                <span style={{ fontSize: '1.4rem' }}>{idx === 0 ? '👑' : '👤'}</span>
                                <span style={{ fontSize: '0.9rem', fontWeight: '800', color: isChosen ? '#FFF' : '#E2E8F0' }}>{name}</span>
                                <span style={{ fontSize: '0.68rem', color: isChosen ? '#38BDF8' : 'var(--text-dim)', fontWeight: '700', letterSpacing: '0.5px' }}>
                        ...
                        <div style={{ textAlign: 'center', padding: '16px', background: 'rgba(255,255,255,0.03)', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)' }}>
                          <p style={{ fontSize: '0.86rem', color: 'var(--text-secondary)', margin: 0 }}>
                            Team Participant Profile Ready
                          </p>
                        </div>
                      )}
                    </div>
