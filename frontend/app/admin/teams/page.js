    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/admin/teams/${editTeam.id}`, {
        ...
        body: JSON.stringify({
          ...
          teamSize: editTeam.teamSize ? parseInt(editTeam.teamSize, 10) : 1,
                  ...
                  teams.map((team, idx) => {
                    return (
                      <tr key={team.id} style={{ borderBottom: idx === teams.length - 1 ? 'none' : '1px solid rgba(255,255,255,0.04)' }}>
                        ...
                        <td style={{ padding: '16px', textAlign: 'center', color: 'var(--text-secondary)' }}>{team.teamSize === 1 ? '1 member (Solo)' : `${team.teamSize || 2} members`}</td>
        ...
        <div style={{
          ...
          <div className="glass-panel" style={{
            ...
            <form onSubmit={handleUpdate}>
              ...
              <div style={{ marginBottom: '14px' }}>
                <label className="form-label" style={{ fontSize: '0.85rem' }}>Team Size (1 - 2)</label>
                <input type="number" min={1} max={2} value={editTeam.teamSize || 1} onChange={e => setEditTeam({...editTeam, teamSize: parseInt(e.target.value)})} className="form-input" required />
              </div>
